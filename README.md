...
| 係数 | 単位 | 解釈 |
|---|---|---|
| $r_0$ | log-units | $r(t_{\text{2013}}) = e^{r_0}$ |
| $r_1$ | log-units / year | $r$ の対数 slope（年率） |
| $a_0$ | log-units | $\alpha_{\text{nbd}}(t_{\text{2013}}) = e^{a_0}$ |
| $a_1$ | log-units / year | $\alpha_{\text{nbd}}$ の対数 slope |

**$t_{\text{2013}}$ で中心化する理由**:
- $r_0, a_0$ を「2013年時点の値の対数」として直接解釈可能
- 観測期間（2008-2019）のほぼ中央 → 切片 $r_0$ と傾き $r_1$ の事後相関が小さくなり、多重共線性が緩和される
- 非中心化した場合 $\log r(t) = r_0' + r_1' \cdot t$ は $r_0' = r_0 - 2013 \cdot r_1$ となり数値的に極端で不安定
